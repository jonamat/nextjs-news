...
import { IconButton, HStack, Tooltip, Select, Collapse, useColorMode, useToast } from '@chakra-ui/react';
...
const NavButtons: FC<Props> = ({ handleInfo }) => {
  ...
  const handleCountrySelect: ChangeEventHandler<HTMLSelectElement> = (event) => {
    toggleSelect();
  ...
  return (
    <HStack flexGrow={0} spacing={5}>
      ...
      >
        ...
        >
          {status === REQ_STATUS.LOADING && <option disabled>Loading countries...</option>}
          {data?.map((country) => (
      ...
      <Collapse in={!selectOpen} animateOpacity transition={{ exit: { duration: 0.2 }, enter: { duration: 0.2 } }}>
        <Tooltip hasArrow label="Search">
          <IconButton colorScheme="blue" icon={<SearchIcon />} onClick={toggleSelect} aria-label="Search" />
        </Tooltip>
      </Collapse>
